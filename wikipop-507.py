...
finalf = [word[0] for word in parts_of_speech if word[1] == "JJ"]
dictionary = {}
for word in finalf:
...
sample_pos_dict = {}
another_list = (sorted(dictionary, reverse = True, key = lambda x: dictionary[x])[:5])
another_list1 = zip( another_list, (sorted(dictionary.values(), reverse = True, key = lambda x: x)[:5]))
sample_pos_dict['JJ'] = another_list1
